...
class EnteredAmountsNotifierSavings extends StateNotifier<Map<String, double>> {
  EnteredAmountsNotifierSavings() : super({});
...
final enteredAmountsSavingsProvider =
    StateNotifierProvider<EnteredAmountsNotifierSavings, Map<String, double>>(
        (ref) => EnteredAmountsNotifierSavings());

// add this line
class CustomTextEditingControllerSavings extends TextEditingController {
  CustomTextEditingControllerSavings({String initialText = ''})
...
// add this line
final textEditingSavingsControllerProvider = StateProvider.family
    .autoDispose<CustomTextEditingControllerSavings, String>((ref, category) {
  double initialAmount =
      ref.read(enteredAmountsSavingsProvider.notifier).getAmount(category);
  print('Initial amount for $category: $initialAmount'); // Add this line
  return CustomTextEditingControllerSavings(
      initialText: initialAmount == 0 ? '' : initialAmount.toStringAsFixed(2));
});

final oldAmountsProviderSavings = StateNotifierProvider.autoDispose<
    OldAmountsControllerSavings, Map<String, double>>((ref) {
  return OldAmountsControllerSavings();
});

class OldAmountsControllerSavings extends StateNotifier<Map<String, double>> {
  OldAmountsControllerSavings() : super({});
...
final focusNodeProviderSavings =
    StateProvider.family.autoDispose<FocusNode, String>((ref, category) {
  return FocusNode();
});

// ignore: must_be_immutable
class ListSavingsItemWidget extends ConsumerWidget {
  final String category;
  final double amount;

  ListSavingsItemWidget({required this.category, required this.amount});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final enteredAmounts = ref.watch(enteredAmountsSavingsProvider);
    final hasAmountEntered =
        enteredAmounts[category] != null && enteredAmounts[category]! > 0;

    // add this line
    final CustomTextEditingControllerSavings _controller =
        ref.watch(textEditingSavingsControllerProvider(category));

    final FocusNode focusNode =
        ref.watch(focusNodeProviderSavings(category).notifier).state;
      ...
      child: Row(
        ...
        children: [
          CustomButton(
            ...
            onTap: () {
              final FocusNode focusNode =
                  ref.read(focusNodeProviderSavings(category).notifier).state;
                        ...
                        TextField(
                          ...
                          onChanged: (text) {
                            ...
                            if (amount != 0 &&
                              ...
                              ref
                                  .read(oldAmountsProviderSavings.notifier)
